...
```{r, purl = FALSE}
xmas_movies2 <- filter(movieSerie, rating == "PG-13")
xmas_movies_ch <- select(movieSerie2, title:description)
```
...
```{r, purl = FALSE}
xmas_movies_ch <- select(filter(movieSerie, rating == "PG-13"),
                         title:description)
```
...
```{r, purl = FALSE}
xmas_movies %>%
  mutate(score_difference = imdb_rating - tmdb_score) %>% 
  select(imdb_rating, tmdb_score, score_difference)
```
...
both imdb and tmdb (`imdb_rating` plus `tmdb_score`).
...
xmas_movies_total_score <- xmas_movies %>%
  mutate(total_score = imdb_rating + tmdb_score) %>%
...
write_csv(xmas_movies_ch, "../data_output/xmas_movies_changed.csv")
```
